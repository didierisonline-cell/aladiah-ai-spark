...
/** Course/modules view (phone). Program progress + locked/unlocked module cards + continue. */
export default function MobileCourse({ course, chapters, lessonCounts, completed }: Props) {
  ...
  // Sequential unlock: module i is unlocked if it's the first or the previous is completed.
  ...
  return (
    <MobileShell title={course?.title ? (course.title.length > 22 ? course.title.slice(0, 22) + '…' : course.title) : 'Program'} back="/portal/courses" score={pct}>
      <div style={{ padding: '18px 16px 8px', maxWidth: 620, margin: '0 auto' }}>
        ...
        <div style={{ background: 'linear-gradient(150deg,rgba(37,99,235,.26),rgba(124,58,237,.2))', border: '1px solid rgba(99,102,241,.3)', borderRadius: 20, padding: 18, marginBottom: 18 }}>
          ...
          <div style={{ fontSize: 12, color: '#c7d2fe', marginTop: 6 }}>{doneCount} of {total} modules complete</div>
        ...
        <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.12em', textTransform: 'uppercase', color: C.fm, margin: '0 4px 10px' }}>Modules</div>
            ...
            return (
              <button key={ch.id} disabled={!unlocked} onClick={() => unlocked && navigate(`/course/${course.id}/chapter/${ch.id}`)}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: C.fm, letterSpacing: '.06em' }}>MODULE {i + 1}</div>
                  ...
                  <div style={{ fontSize: 11, marginTop: 3, fontWeight: 700, color }}>
                    {done ? 'Completed' : isCurrent ? 'Continue →' : unlocked ? `${lessonCounts[ch.id] || 0} lessons` : 'Locked'}
                  </div>
                </div>
        ...
        {chapters.length === 0 && (
          <div style={{ textAlign: 'center', padding: 36, color: C.fm }}>
            ...
            <div style={{ fontSize: 13.5 }}>Modules are being prepared. Check back soon.</div>
          </div>
        )}
      </div>
    </MobileShell>
