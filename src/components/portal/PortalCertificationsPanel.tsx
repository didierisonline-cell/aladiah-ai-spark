...
const PortalCertificationsPanel = () => {
  ...
  <div>
    <div style={{marginBottom:'1.5rem'}}>
      <div style={{fontSize:'10px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:'var(--muted-foreground)',marginBottom:'0.75rem'}}>✅ {t('certpanel.earned')}</div>
      {[{code:'AEX',name:'Aladiah Certified Expert™',spec:'AI Agent Engineer',date:'May 26, 2026',id:'ACE-2026-0047',col:'#F5B81A',bg:'rgba(245,184,26,0.12)',bdr:'rgba(245,184,26,0.3)'},{code:'APR',name:'Aladiah Professional™',spec:'Scrum Master',date:'Mar 12, 2026',id:'APR-2026-0021',col:'#C0C0C0',bg:'rgba(192,192,192,0.1)',bdr:'rgba(192,192,192,0.3)'}].map(c=>(
        <div key={c.code} style={{display:'grid',gridTemplateColumns:'56px 1fr auto',gap:'1.25rem',alignItems:'center',background:'var(--card)',border:`1px solid ${c.bdr}`,borderRadius:'0.75rem',padding:'1.25rem 1.5rem',marginBottom:'0.6rem'}}>
          ...
          <div><div style={{fontSize:'14px',fontWeight:700,color:c.col}}>{c.name}</div><div style={{fontSize:'12px',color:'var(--muted-foreground)'}}>{c.spec} · {t('certpanel.verified')} {c.date} · ID: {c.id}</div></div>
          <div style={{display:'flex',gap:'0.5rem'}}>
            <button style={{fontSize:'11px',fontWeight:600,padding:'5px 12px',borderRadius:'0.45rem',background:'transparent',border:'1px solid var(--border)',color:'var(--muted-foreground)',cursor:'pointer'}}>🔗 {t('certpanel.share')}</button>
            <button style={{fontSize:'11px',fontWeight:600,padding:'5px 12px',borderRadius:'0.45rem',background:'transparent',border:'1px solid var(--border)',color:'var(--muted-foreground)',cursor:'pointer'}}>📄 {t('certpanel.download')}</button>
          </div>
        </div>
      ))}
    </div>
    <div style={{marginBottom:'1.5rem'}}>
      <div style={{fontSize:'10px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:'var(--muted-foreground)',marginBottom:'0.75rem'}}>🔄 {t('certpanel.in_progress')}</div>
    ...
    <div>
      <div style={{fontSize:'10px',fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:'var(--muted-foreground)',marginBottom:'0.75rem'}}>🔒 {t('certpanel.locked')}</div>
