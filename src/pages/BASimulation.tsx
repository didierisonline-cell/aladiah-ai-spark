...
import DiscoveryEngagement from '@/components/simulation/ba/DiscoveryEngagement';
import {
...
export default function BASimulation() {
  ...
  if (started) return <DiscoveryEngagement onExit={() => setStarted(false)} />;
  ...
  return (
    <div className="min-h-screen bg-[#0B111E] text-foreground">
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* CTA — launch the interactive Discovery Engagement */}
        <div className="flex items-center justify-between rounded-2xl border border-secondary/30 bg-secondary/5 p-5">
          <div className="flex items-center gap-2 text-sm text-foreground/80">
            ...
            Interview the stakeholders, gather evidence, and recommend — there’s no single answer key.
          </div>
          <button onClick={() => setStarted(true)} className="px-5 py-2.5 rounded-xl bg-secondary text-background text-sm font-semibold hover:opacity-90">
            Begin Discovery →
          </button>
        </div>
      </div>
