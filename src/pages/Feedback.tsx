  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      ...
      if (missingIds.length > 0) {
        const { data: mp } = await supabase.from('public_profiles' as any).select('user_id, full_name').in('user_id', missingIds);
      ...
      setPosts(postsData.map(post => ({
        ...
        replies: (repliesData || []).filter(r => r.post_id === post.id).map(r => ({
          ...r,
      ...
      <main className="container mx-auto px-4 py-8 pt-16">
        ...
        {loadingPosts ? (
          ...
          <div className="space-y-4">
            {posts.map(post => (
              <PostCard key={post.id} post={post} currentUserId={user!.id} onReplyAdded={loadPosts} />
            ))}
          </div>
        )}
      </main>
