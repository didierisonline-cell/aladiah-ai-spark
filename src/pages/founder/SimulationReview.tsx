...
interface SimRow { id: string; user_id: string; score: number | null; grade: string | null; readiness_score: number | null; recommendation: string | null; created_at: string; report: any; story: any }
...
export default function SimulationReview() {
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await sb.from('ba_simulations')
        .select('id,user_id,score,grade,readiness_score,recommendation,created_at,report,story')
  ...
  const cohort = useMemo(() => {
    ...
    return {
      count: rows.length,
      avgScore: avg(rows.map(r => r.score || 0)),
      avgReadiness: avg(rows.map(r => r.readiness_score || 0)),
  ...
  return (
    <div className="min-h-screen bg-[#0B111E] text-foreground">
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-5">
        ...
        <h1 className="text-2xl font-display font-bold">Simulation Review — Discovery Engagement</h1>
            ...
            {selRow && detail && (
              <>
                <div className="rounded-2xl border border-border/40 bg-card/60 p-4">
                  <div className="flex items-center gap-2 mb-2"><BarChart3 className="w-4 h-4 text-secondary" /><span className="text-xs font-bold uppercase tracking-wide text-foreground/70">Scoring breakdown</span></div>
